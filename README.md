...
这个项目直接选了基于 Tree-sitter 的路线，并把 grammar、query、scanner、detector 和 nested runtime 都尽量放在仓库里。这样做不是为了追求某种“更标准”的架构，而是为了把更多中间层暴露出来，方便继续实验这类更依赖结构语义和嵌套分发的高亮场景。
...
- 跑测试：`just test`
- CI 会先在 Linux 上执行 `cargo nextest run` 测试，再继续 release build matrix
