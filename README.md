...
TTY 下的普通渲染输出统一进入内建 alternate-screen viewer，从文件开头开始显示，并按终端宽度展开后的 display row 滚动；支持 `j/k`、方向键、PageUp/PageDown、Home/End、`/` 普通文本搜索、`n/N` 前后匹配和 `q` 退出。`kat` 不再启动外部 `PAGER`，也不再提供 `--paging` 开关。stdout 不是 TTY 时仍直接输出完整内容，适合 pipe/redirect。
