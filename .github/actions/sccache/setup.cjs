...
async function main() {
  ...
  await exportVariable("SCCACHE_GHA_ENABLED", "true");
  await exportVariable("RUSTC_WRAPPER", "sccache");
  await exportVariable("CC", "sccache cc");
  await exportVariable("CXX", "sccache c++");
