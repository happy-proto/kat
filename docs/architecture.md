...
- YAML 模板同样使用 projection-based injection：`*.yaml.j2` / `*.yml.j2` 会把 Jinja expression 投影成合法的 YAML 标量，同时保留完整文档结构和回到原文的 source map。不能只拼接离散 `content` 节点，否则第一个模板表达式就可能让后续 mapping key、scalar 与 block 结构全部失去 YAML 语义。
