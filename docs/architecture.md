...
- 模板宿主统一使用 projection-based injection：host resolver 会按模板 AST 顺序保留宿主 `content`、把输出 expression 替换成由目标 runtime 选择的合法占位符，并忽略不产生输出的 control/comment 节点。HTML / XML / YAML / SQL / CSS / JavaScript / CoffeeScript 因而都能解析一份结构连续的 virtual source，同时通过 source map 保留 Jinja / Twig / ERB 等模板语法的原文高亮；不能只拼接离散 `content` 节点，否则任意模板 expression 都可能改变 token 边界或破坏后续宿主结构。各目标 runtime 的占位符策略保留独立入口，后续出现语言特有边界时不需要把特判下沉到通用 projection 或 renderer。
