...
- 需要验证真实终端边界时，优先使用 `ghostty-e2e` feature 下的 PTY + `libghostty-vt` 测试：测试先在固定尺寸 PTY 里运行真实 `kat` 二进制，再把输出字节交给 Ghostty 的无头终端模型断言 screen/cell 结果；交互式场景可以在进程退出前发送输入或 resize PTY。该测试依赖 `libghostty-vt-sys` 的 Ghostty native build，所需 Zig 版本由仓库根 [mise.toml](../mise.toml) 统一管理；默认 `just test` 不启用它，CI 通过独立 `ghostty-e2e` job 显式运行。
