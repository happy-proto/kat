...
- 模板宿主已经补齐到统一模型：`ERB` / `EEx` / `JSP` / `ASP` / `ADP` 共享同一套 `<% ... %>` AST，`Jinja` / `Twig` 则保留各自模板 AST；它们的宿主内容都会按复合后缀分发到 `HTML` / `XML` / `YAML` / `SQL` / `CSS` / `JavaScript` / `CoffeeScript` runtime，并通过共享的完整文档 projection 以合法占位符替代输出 expression，不再把离散 content 片段直接拼接后交给宿主 parser。
