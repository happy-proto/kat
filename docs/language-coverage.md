...
- 模板宿主已经补齐到统一模型：`ERB` / `EEx` / `JSP` / `ASP` / `ADP` 共享同一套 `<% ... %>` AST，`Jinja` / `Twig` 则保留各自模板 AST；它们的宿主内容都会按复合后缀分发到 `HTML` / `XML` / `YAML` / `SQL` / `CSS` / `JavaScript` / `CoffeeScript` runtime，并通过共享的完整文档 projection 以合法占位符替代输出 expression，不再把离散 content 片段直接拼接后交给宿主 parser。Jinja 的 7 种宿主 profile 现在都有真实 fixture，并在 `testdata/showcase/jinja/` 下提供对应的人工展示样例；HTML showcase 还同时覆盖 `<script>` / `<style>` 的递归注入。
- `ActionScript`、`Ada`、`AppleScript`、`Assembly (.s/.S)`、`AsciiDoc`、`Authorized Keys`、`AWK`、`BibTeX` 这一轮也都补上了独立 runtime、fixture 和 detector；其中 `Ada` 直接走公开 crate，另外这批过去仍留在主仓库里的 vendored parser 也已经外置到 `kat-parsers` bundle，主仓库只保留 kat 侧 query / detector 资产。`ASP` 也补齐了此前缺失的 `.asa` 入口。
- `PEM` 与 `OpenPGP ASCII armor` 现在作为轻量 pseudo-runtime 接入，覆盖证书 / CSR / CRL / 公私钥文本封装、PGP 公钥 / 私钥 / 消息 / 签名 / cleartext signed message。它们不解析 ASN.1 或 OpenPGP packet，只高亮稳定文本边界、label、header、base64 body 与 checksum，避免为终端查看场景引入重依赖。
- `Cabal`、`CFML`、`Clojure`、`CoffeeScript`、`Crystal`、`D`、`Elm`、`Erlang`、`F#`、`Fortran` 这一轮也都补上了独立 runtime、fixture 和 detector；其中 `CFML`、`Clojure`、`D`、`Elm`、`Erlang`、`F#`、`Fortran` 直接走 crate-backed parser，`Cabal`、`CoffeeScript`、`Crystal`、`CMakeCache`、`Command Help`、`CpuInfo`、`debsources`、`Fortran Namelist`、`fstab` 这批没有合适公开 crate 或仍带本地语义包装的语言，则统一改为走外部 `kat-parsers` bundle 提供的预生成 parser。`coffee.erb` 也已重新接回统一模板宿主分发链路；`CMake` 生成的 `*.h.in` / `*.hpp.in` 头文件模板、`cron.d/*` 与 `/var/mail/*` / `/var/spool/mail/*` 也已纳入 detector。
- `Scala` 当前使用 crate-backed parser；升级到 `tree-sitter-scala` 0.26.2 后，本地 highlights query 已同步适配新的 `end_marker` AST，并补齐 `extension` 软关键字、`forSome`、冒号参数运算符与 XML literal capture，避免 parser 与 query 版本错配导致 runtime 初始化失败。
...
- `CSV`、`SML`、`Solidity`、`Strace`、`Stylus`、`syslog`、`SystemVerilog`、`varlink`、`Verilog`、`VimHelp`、`Vyper`、`WGSL` 这一轮也都补上了独立 runtime、fixture 和 detector；其中 `Solidity` / `Verilog` / `SystemVerilog` / `varlink` 直接走 crate-backed parser，`CSV`、`SML`、`Strace`、`Stylus`、`syslog`、`VimHelp`、`Vyper`、`WGSL` 这批暂时没有合适公开 crate 的语言则统一走外部 `kat-parsers` bundle，不再继续让主仓库自己生成 vendored parser。
...
| Ada | 🟨 基础 | 已支持 `.adb` / `.ads` / `.gpr`；当前 `ada` runtime 已改为 crate-backed parser，并保留本地 highlights/locals query 作为集成资产。当前依赖的 `tree-sitter-ada` 仍固定在 git revision，而不是 crates.io `0.1.0`，因为该 revision 已包含 `parallel_block_statement`、`procedural_iterator` 与 `parallel for` 相关语法支持；fixture 已覆盖基础 source file。 | 生态里已有独立 Ada Tree-sitter grammar/crate，可长期沿着 query 细化继续演进。 | 对这门语言来说，先把 parser 迁到 crate-backed 再接入 runtime，是比继续保留本地 parser 源更长期可维护的路径；后续主要工作会是把 capture 语义压实。 |
...
| CoffeeScript | 🟨 基础 | 已支持 `.coffee` / `.cson` 与 `Cakefile`；当前 runtime 重新走外部 `tree-sitter-kat-parsers` bundle 提供的预生成 parser，主仓库只保留本地 highlights / injections query。fixture 已覆盖顶层文件与 `coffee.erb` 模板宿主分发。 | 生态里已有独立 CoffeeScript grammar；成熟实现通常也会把 `coffee` source 与模板宿主里的 CoffeeScript 内容分开建模但复用同一 runtime。 | 当前这层已经把“重新接回支持”与“主仓库去构建负担”同时落地；后续若继续做深，重点会是 query 细化，而不是 parser 生成链路。 |
| Crystal | 🟨 基础 | 已支持 `.cr`；当前 runtime 重新走外部 `tree-sitter-kat-parsers` bundle 提供的预生成 parser，主仓库只保留本地 highlights / injections query。fixture 已覆盖最小 source file。 | 生态里已有独立 Crystal grammar；成熟实现通常会把它作为 Ruby 风格但独立 AST 的语言单独处理。 | 当前这层已经把过去的高构建成本 vendored grammar 迁回到外部 parser bundle；后续若继续投入，价值主要来自更细的 Crystal 语义 capture，而不是重新折腾构建模型。 |
| TOML | 🟩 精细 | 已支持 `.toml`、`Cargo.toml`、`Cargo.lock` 和 `uv.lock`；Markdown `+++` frontmatter 已注入 TOML；fixture / showcase 已覆盖 quoted key、escape、datetime、inline table、array table，以及 lockfile 检测。除此之外，`prek.toml` 现已作为 TOML profile 接入宿主级 resolver：本地 `[[repos.hooks]]` 下 `language = "system"` 的 `entry` 会按 inline shell command 注入 Bash runtime，像 `cargo fmt --check`、`cargo clippy ...`、`cargo nextest run ...` 这类命令串可以直接复用现有 shell 高亮，而不会把其它非 `system` hook 误当成 shell。Dracula 语义上，TOML key 现按配置语言 key 使用青色，datetime 走橙色值语义。 | 本地 `zed` 文档里有 TOML 语言支持，但当前代码树里没有像其它 grammar 那样直接暴露出可比对的 query 资产。 | 对 `kat` 而言，TOML 这层已经非常完整；现阶段新增工作更多会落在类似 `prek.toml` 这种宿主 profile，而不是 TOML 基础语法本身。 |
| YAML | 🟩 精细 | 已支持 `.yaml` / `.yml`；Markdown `---` frontmatter 已注入 YAML；fixture / showcase 已覆盖 anchor / alias / tag / merge key / block scalar / GitHub Actions workflow。`*.yaml.j2` / `*.yml.j2` 会按复合后缀进入 Jinja + YAML profile，并通过 projection-based injection 以合法标量替代模板 expression，避免离散内容拼接破坏后续 mapping 结构。当前除 `actions/github-script` 的 JavaScript 注入外，GitHub Actions workflow / `action.yml` 这类 profile 也已接入宿主级 resolver：`run` block 会按同级 `shell` 分发到 `bash` / `zsh` / `fish` / `python` 等已注册 runtime，`${{ ... }}` expression 与 `uses: owner/repo@ref` 也已有专门语义高亮。Dracula 语义上，YAML alias / anchor 单独走绿色斜体下划线，key 走配置语言 key 的青色。 | 本地 `zed` 有独立 YAML grammar 包，并额外做了 GitHub Actions `actions/github-script` -> JavaScript 注入；这部分已被 `kat` 对齐进统一 runtime。 | YAML 现在已经从“基础接入”升级到“场景化精细支持”；后续剩余工作更多是继续扩充被注入子语言，而不是 YAML 宿主层本身。 |
...
| Python | 🟩 精细 | 已支持 `.py`；当前 query 已覆盖 decorator / decorator call、builtin call、builtin type、constructor method、annotation string、`self` / `cls`、`isinstance` / `issubclass`，并把模块 / 类 / 函数 / `decorated_definition` 下的 docstring 统一分发到仓库内独立的 `python_docstring` pseudo-runtime，而不是默认退回 Markdown。这个 runtime 会先做 style auto-detect，再按 `plain` / `reST` / `Google` / `NumPy` profile 产出结构高亮；当前已覆盖 section header、reST field list / inline role、NumPy underline section、Google 参数条目、doctest prompt、doctest 里的 Python 子 runtime，以及 docstring block 几何。同时还补上了 `# sql` 注释块、`.execute(...)` / `.executemany(...)` / `.executescript(...)` 调用点，以及 `re.*` / `regex.*` 调用点的 SQL/Regex 注入；raw string 与 backslash-escaped string 也都会先 decode 再进入共享 runtime。fixture / showcase 已同步改成真实 docstring style 样例。 | 本地 `zed` 的 Python grammar query 仍然还会继续覆盖更多注入场景，例如 SQL string injection；而 PyCharm 一类 IDE 则会把 docstring format 当成独立能力面，支持 `reST` / `Google` / `NumPy` 而不是默认 Markdown。 | 在当前终端高亮目标下，Python 已经达到仓库内精细语言的标准；继续推进时，真正有价值的方向会更多落在 docstring 语义绑定、doctest 子 runtime 和更多嵌入语言边界，而不是继续把 docstring 当通用 markup 注入。 |
...
| Fish | 🟦 中等 | 已接入独立 `fish` runtime，支持 `.fish`、`config.fish`、`fishfile`、`fish_variables` 与 fish shebang；当前 query 已覆盖 comment、shebang、string、escape、number、operator、function definition、command/builtin、flag/option、variable expansion、fish special variable、`for` loop 变量、command substitution、glob，以及 `case` pattern 的 fish-specific 语义；这一轮除了继续扩 builtin 覆盖面，也开始由共享 shell semantic layer 处理 `status` / `string` 这类 builtin family 的 subcommand、`function --argument-names` / `--on-variable` 这类函数元数据参数，以及 list access 的结构化语义。`fish_variables` 则额外走 `fish` runtime 下的专用 profile / semantic overlay：会把 `SETUVAR` 记录头、变量名、键值分隔符、`\xNN` 转义、`\x1e` 列表分隔，以及 `fish_user_paths` / `*_PATH*` 这类路径列表值单独提亮，而不是把整个文件误当成普通 fish 脚本。fixture / showcase 已覆盖独立文件场景；Justfile 的 fish shebang recipe 也已真正落到 fish runtime，而不再退回 bash。 | 本地生态里已有独立 grammar。 | 这一层已经不只是“能识别成 fish”，而是开始有 shell-specific 结构语义，并且能对 fish 自己的持久化变量文件做 profile-aware 细化；与 Bash 样板语言相比，剩余空间主要收敛到更细的 expansion / list access / builtin-family 细节，而不是 runtime 是否独立。 |
